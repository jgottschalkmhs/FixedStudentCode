    ...
    while dodgy_sf == "yes":
        ...
        if scale_factor < 0.25:
            dodgy_sf = input("Warning: This scale factor is very small "
                             "and you might struggle to accurately weigh the ingredients. \n"
                             "Do you want to keep going (type 'no' to change your desired serving size)").lower()
        elif scale_factor > 4:
            dodgy_sf = input("Warning: This scale factor is quite large - you might have issues with mixing bowl volumes and oven sapce. \n"
                             "Do you want to keep going (type 'no' to change your desired serving size)").lower()
        else:
            dodgy_sf = "no"
...
def unit_checker(raw_unit):
    ...
    if unit_tocheck == "":
        ...
        return "g"
    elif unit_tocheck.lower() in dls:
        return "dls"
...
for recipe_line in full_recipe:
    ...
    if num_spaces > 1:
        ...
        if amount[1] == "yes":
            ...
            # if the ingredient is not in the lsit, leave the unit as ml
            else:
